...
class XanoClient {
  ...
  // Data collection webhook endpoint (replaces n8n workflow)
  async submitDataCollection(data: {
    ...
    return this.requestWithRetry('/api:owKhF9pX/webhook/data_collection_n8n', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
  ...
  async getAllMessages(conversationId: number): Promise<XanoApiResponse<any>> {
    return this.requestWithRetry(`/api:owKhF9pX/conversation/all_messages/conversation_id?conversation_id=${conversationId}`, {
      method: 'GET',
    });
  }
}
