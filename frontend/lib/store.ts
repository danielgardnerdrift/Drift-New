...
> = (set, get) => ({
      ...
      sendMessage: async (message: string) => {
        ...
        if (state.isTyping) {
          console.log('🟣 [store.sendMessage] BLOCKED - Already sending a message');
          return;
        }
      ...
      streamResponse: (response: StreamResponse) => {
        set((state) => {
          ...
          return {
            ...
            workflowId: response.workflow_id || state.workflowId,
            collectedData: response.collected_data ? 
              { ...state.collectedData, ...response.collected_data } : 
              state.collectedData,
            isTyping: response.done ? false : state.isTyping
          }
        })
      },
...
// Selectors for efficient component rendering
export const useChatMessages = () => useChatStore((state) => state.messages)
export const useIsTyping = () => useChatStore((state) => state.isTyping)
export const useWorkflowId = () => useChatStore((state) => state.workflowId)
export const useCollectedData = () => useChatStore((state) => state.collectedData)
export const useSessionData = () => useChatStore((state) => state.sessionData)
export const useChatError = () => useChatStore((state) => state.error)
export const useConversationId = () => useChatStore((state) => state.conversationId)
export const useCurrentField = () => useChatStore((state) => state.currentField)

// Action selectors
export const useChatActions = () => {
  const sendMessage = useChatStore((state) => state.sendMessage)
  const streamResponse = useChatStore((state) => state.streamResponse)
  const updateCollectedData = useChatStore((state) => state.updateCollectedData)
  const setConversationId = useChatStore((state) => state.setConversationId)
  const setWorkflowId = useChatStore((state) => state.setWorkflowId)
  const setWorkflowStatus = useChatStore((state) => state.setWorkflowStatus)
  const setSessionData = useChatStore((state) => state.setSessionData)
  const setIsTyping = useChatStore((state) => state.setIsTyping)
  const setError = useChatStore((state) => state.setError)
  const setCurrentField = useChatStore((state) => state.setCurrentField)
  ...
  useChatStore.subscribe((state, prevState) => {
    if (state.messages.length !== prevState.messages.length) {
      console.log('💬 Chat messages updated:', {
        ...
        addedMessages: state.messages.length - prevState.messages.length
      })
    }
    
    if (state.workflowId !== prevState.workflowId) {
      console.log('🔄 Workflow changed:', {
        from: prevState.workflowId,
        to: state.workflowId
      })
    }
    ...
    if (state.isTyping !== prevState.isTyping) {
      console.log('⌨️ Typing state:', state.isTyping)
    }
  })
}